...
// Cache for subscriber tiers (populated from subscriber-latest events)
// Structure: Map<username, {tier: string, timestamp: number}>
let subscriberTierCache = new Map();
const TIER_CACHE_DURATION = 60 * 60 * 1000; // Cache for 1 hour

// Subscriber tier multipliers
function getSubscriberMultiplier(badges, eventData) {
  if (!fieldData.enableSubBonus || fieldData.enableSubBonus === 'false') {
    return 1.0; // No bonus if feature disabled
  }

  // Debug: Log event data to see tier information
  console.log('[Heist Widget] Event data received:', JSON.stringify(eventData));

  // First check if user is a subscriber at all
  const isSubscriber = badges && badges.find(b => b.type === 'subscriber');
  
  if (!isSubscriber) {
    console.log('[Heist Widget] No subscriber badge found');
    return 1.0; // Not a subscriber
  }

  // Tier information is in eventData.tier (Twitch standard)
  // Values: 'prime', '1000', '2000', '3000'
  const tier = eventData?.tier || '';
  
  console.log('[Heist Widget] Subscriber tier from event:', tier);
  
  // Check tier from event data
  const tierStr = tier.toString().toLowerCase();
  
  if (tierStr === '3000' || tierStr === 'tier3' || tierStr === 'tier 3') {
    if (fieldData.enableTier3Bonus === 'false') return 1.0;
    console.log('[Heist Widget] Tier 3 subscriber bonus:', fieldData.subTier3Multiplier);
    return parseFloat(fieldData.subTier3Multiplier || 1.5);
  } else if (tierStr === '2000' || tierStr === 'tier2' || tierStr === 'tier 2') {
    if (fieldData.enableTier2Bonus === 'false') return 1.0;
    console.log('[Heist Widget] Tier 2 subscriber bonus:', fieldData.subTier2Multiplier);
    return parseFloat(fieldData.subTier2Multiplier || 1.3);
  } else if (tierStr === '1000' || tierStr === 'tier1' || tierStr === 'tier 1') {
    if (fieldData.enableTier1Bonus === 'false') return 1.0;
    console.log('[Heist Widget] Tier 1 subscriber bonus:', fieldData.subTier1Multiplier);
    return parseFloat(fieldData.subTier1Multiplier || 1.2);
  } else if (tierStr === 'prime' || tierStr === '0') {
    if (fieldData.enablePrimeBonus === 'false') return 1.0;
    console.log('[Heist Widget] Prime subscriber bonus:', fieldData.subPrimeMultiplier);
    return parseFloat(fieldData.subPrimeMultiplier || 1.15);
  }
  
  // If we couldn't detect tier, no bonus
  console.log('[Heist Widget] Could not determine subscriber tier from event data, no bonus applied');
  return 1.0;
}

function getSubscriberTierName(badges, eventData) {
  ...
  console.log('[Heist Widget] Getting tier name from event:', tier);
  
  // Check tier from event data
  const tierStr = tier.toString().toLowerCase();
  if (tierStr === '3000' || tierStr === 'tier3' || tierStr === 'tier 3') return 'Tier 3';
  if (tierStr === '2000' || tierStr === 'tier2' || tierStr === 'tier 2') return 'Tier 2';
  if (tierStr === '1000' || tierStr === 'tier1' || tierStr === 'tier 1') return 'Tier 1';
  if (tierStr === 'prime' || tierStr === '0') return 'Prime';
...
function updateParticipantDisplay() {
  ...
  // Calculate total bank (sum of all bets)
  if (bankEl) {
    const totalBank = heistState.participants.reduce((sum, p) => sum + p.amount, 0);
    const displayCurrency = currencyName || 'points';
    bankEl.textContent = `${totalBank.toLocaleString()} ${displayCurrency}`;
  }
  ...
  if (listEl) {
    ...
    listEl.innerHTML = heistState.participants.map(p => {
      let subBadge = '';
      // Show tier badge if subscriber bonuses are enabled and user has a tier
      if (fieldData.enableSubBonus === 'true' && p.subTier && p.subMultiplier > 1.0) {
        const tierColors = {
          'Prime': '#8e44ad',
          'Tier 1': '#3498db',
          'Tier 2': '#e74c3c',
        ...
        const color = tierColors[p.subTier] || '#3498db';
        subBadge = `<span style="background: ${color}; padding: 2px 6px; border-radius: 4px; font-size: 10px; margin-left: 4px; font-weight: bold;">${p.subTier}</span>`;
      }
      return `
        <div class="participant-item">
          <span class="participant-name">${p.username}${subBadge}</span>
...
window.addEventListener('onWidgetLoad', async function(obj) {
  ...
  // Mark as initialized after we have field data
  isInitialized = true;
  ...
  // Send welcome message if enabled (only in live mode, not editor)
  if (fieldData.enableWelcome === 'true' && fieldData.msgWelcome) {
    // Check if we're in editor mode by looking for the 'er' parameter in URL
    const urlParams = new URLSearchParams(window.location.search);
    const isEditor = urlParams.has('er') || window.location.href.includes('/editor');
    
    if (!isEditor) {
      sendChatMessage(fieldData.msgWelcome);
    } else {
      console.log('[Heist Widget] Skipping welcome message in editor mode');
    }
  }
});
  ...
  window.addEventListener('onEventReceived', function(obj) {
    ...
    if (listener === 'subscriber-latest') {
      ...
      if (username && tier) {
        ...
        console.warn('[Heist Widget] ⚠️ Subscriber event missing username or tier:', event);
      }
...
// Fetch user's subscriber tier from cache (populated by subscriber-latest events)
// StreamElements API does not provide tier info in user/points endpoints
// We must listen to subscriber-latest events and cache the tier information
const getUserSubscriberTier = (username) => {
  ...
  console.log(`[Heist Widget] ${username} tier not in cache - may need to sub/resub to populate`);
  return null;
};
...
async function joinHeist(username, userId, amount, risk, event) {
  ...
  console.log(`[Heist Widget] ${username} tier from cache:`, userTier);
  
  // Create event data with the tier information
  const tierEventData = {
    badges: event.data.badges,
    tier: userTier // This will be '1000', '2000', '3000', 'prime', or null
  };
  
  // Get subscriber bonus multiplier using the cached tier
  ...
  if (subMultiplier > 1.0) {
    console.log(`[Heist Widget] ${username} has ${subTier} bonus: ${subMultiplier}x multiplier`);
  }
  ...
  const participant = {
    ...
    amount,
    risk: risk || 'low',
    subMultiplier, // Store subscriber multiplier
    subTier // Store tier for display
  };
  ...
  // Create join message (no tier info in chat, only in visual overlay)
...
async function executeHeist() {
  ...
  heistState.participants.forEach(participant => {
    ...
    if (success) {
      ...
      // Apply subscriber bonus multiplier
      const finalMultiplier = multiplier * (participant.subMultiplier || 1.0);
      const winnings = Math.floor(participant.amount * finalMultiplier);
      ...
      results.push({
        ...
        success: true,
        amount: winnings,
        subBonus: participant.subMultiplier > 1.0 ? participant.subMultiplier : null,
